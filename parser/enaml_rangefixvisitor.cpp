...
void RangeFixVisitor::visitFunctionDefinition(Python::FunctionDefinitionAst* node) {
    ...
    {
        const int asyncOffset = node->async ? indexOf(node->startLine, QStringLiteral("async")) + 5: 0;
        const bool isOverride = indexOf(node->startLine, QStringLiteral("=>")) > 0;
        const bool isNotification = indexOf(node->startLine, QStringLiteral("::")) > 0;
        const bool isUpdate = indexOf(node->startLine, QStringLiteral("<<")) > 0;
...
void RangeFixVisitor::visitEnamlDef(EnamlDefAst* node) {
    ...
    // Fix base
    {
        ...
        const int startCol = firstNonSpace(node, indexOf(node->startLine, QStringLiteral("(")) + 1);
    ...
    const int selfStart = indexOf(node->startCol, QStringLiteral(":"));
...
void RangeFixVisitor::visitAssignment(Python::AssignmentAst* node)
{
    ...
    if (dynamic_cast<EnamlDefAst*>(node->parent))
    {
        if (auto target = dynamic_cast<Python::NameAst*>(node->targets.at(0))) {
            const int offset = indexOf(node->startLine, QStringLiteral(":"));
            bool isSelf = target->identifier->value == QStringLiteral("self");
        ...
        if (auto value = dynamic_cast<Python::NameAst*>(node->value)) {
            if (value->identifier->value == QStringLiteral("self")) {
                const int startCol = indexOf(node->parent->startLine, QStringLiteral(":"));
...
void RangeFixVisitor::visitEnamlAlias(AliasAst* node) {
    const int startCol = firstNonSpace(node, indexOf(node->startLine, QStringLiteral("alias")) + 5);
    if (startCol > 0)
    {
        ...
        const int colonPos = indexOf(node->startCol, QStringLiteral(":"));
