...
QList<QString> EnamlDUChainTest::FindPyFiles(QDir& rootDir)
{
    ...
    rootDir.setNameFilters(QStringList()<< QStringLiteral("*.py") << QStringLiteral("*.enaml")); // We only want .enaml files
...
void EnamlDUChainTest::init()
{
    ...
    QString correctionFileDir = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("kdevpythonsupport/correction_files"), QStandardPaths::LocateDirectory);
...
void EnamlDUChainTest::initShell()
{
    ...
    auto doc_url = QStandardPaths::locate(
...
ReferencedTopDUContext EnamlDUChainTest::parse(const QString& code)
{
    TestFile* testfile = new TestFile(
        QStringLiteral("#!/usr/bin/python\n") + code + QStringLiteral("\n"),
        QStringLiteral("enaml"),
        nullptr,
        testDir.absolutePath().append(QStringLiteral("/"))
    );
...
public:
    ...
    void checkType(Python::Ast *node, Python::Identifier *identifier) {
        if ( identifier->value != QStringLiteral("checkme") ) return;
...
void EnamlDUChainTest::testTypes()
{
    ...
    ReferencedTopDUContext ctx = parse(QString::fromUtf8(code.toUtf8()));
    QVERIFY(ctx);
    QVERIFY(m_ast);
